...
ibm_sim = function(n_sim,             # Number of simulations
                   n_years,           # Number of years per simulations
  ...
  for(i in 2:n_years){
    ...
    if(vital_rate == "survival"){
      ...
      summerT_growth = unique(droso_anthropogenic$summerT_unscaled[which(droso_anthropogenic$site == population &
                                                                           droso_anthropogenic$time == year_obs)])
      
      prevwinterR_flowering = unique(droso_anthropogenic$prevwinterR_unscaled[which(droso_anthropogenic$site == population &
                                                                                      droso_anthropogenic$time == year_obs)])
      
      prevfallR_nbFlowers = unique(droso_anthropogenic$prevfallR_unscaled[which(droso_anthropogenic$site == population &
                                                                                  droso_anthropogenic$time == year_obs)])
      
      prevwinterT_seedlingSize = unique(droso_anthropogenic$prevwinterT_unscaled[which(droso_anthropogenic$site == population &
                                                                                         droso_anthropogenic$time == year_obs)])
      ...
      summerT_growth = unique(proj_clim$summerT_unscaled[which(proj_clim$population == population &
                                                                 proj_clim$year == year &
                                                                 proj_clim$model == climate_model)])
      
      # Climate variables for other vital rates are current (control) values
      summerT_survival = unique(droso_anthropogenic$summerT_unscaled[which(droso_anthropogenic$site == population &
                                                                             droso_anthropogenic$time == year_obs)])
      fallR_survival = unique(droso_anthropogenic$fallR_unscaled[which(droso_anthropogenic$site == population &
                                                                         droso_anthropogenic$time == year_obs)])
      ...
      prevwinterR_flowering = unique(proj_clim$prevwinterR_unscaled[which(proj_clim$population == population &
                                                                            proj_clim$year == year &
                                                                            proj_clim$model == climate_model)])
      ...
      prevfallR_nbFlowers = unique(proj_clim$prevfallR_unscaled[which(proj_clim$population == population &
                                                                        proj_clim$year == year &
                                                                        proj_clim$model == climate_model)])
    ...
    if(nrow(data_droso) > 0){
      ...
      data_droso$flowering = rbinom(n = nrow(data_droso), 
                                    flowering_function(size = data_droso$size_unscaled,
                                                       ...
                                                       prevwinterR = prevwinterR_flowering,
                                                       year = year_obs,
      ...
      if(length(data_droso$nbFlowers[which(data_droso$flowering == 1)]) > 0){
        ...
        if(any(data_droso_sub$nbFlowers > 0)){
          ...
          seedling_size_parameters = seedling_size_function(prevwinterT = prevwinterT_seedlingSize,
...
ibm_monteratorero_canesm5_sensitivity_survival = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_canesm5_sensitivity_survival, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_canesm5_sensitivity_survival)
...
# Run projections in parallel
ibm_monteratorero_canesm5_sensitivity_growth = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_canesm5_sensitivity_growth, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_canesm5_sensitivity_growth)
...
# Run projections in parallel
ibm_monteratorero_canesm5_sensitivity_flowering = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_canesm5_sensitivity_flowering, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_canesm5_sensitivity_flowering)
...
# Run projections in parallel
ibm_monteratorero_canesm5_sensitivity_nbFlowers = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_canesm5_sensitivity_nbFlowers, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_canesm5_sensitivity_nbFlowers)
...
# Run projections in parallel
ibm_monteratorero_canesm5_sensitivity_seedlingSize = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_canesm5_sensitivity_seedlingSize, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_canesm5_sensitivity_seedlingSize)
...
# Run projections in parallel
ibm_monteratorero_ec_earth3_sensitivity_survival = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_ec_earth3_sensitivity_survival, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_ec_earth3_sensitivity_survival)
...
# Run projections in parallel
ibm_monteratorero_ec_earth3_sensitivity_growth = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_ec_earth3_sensitivity_growth, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_ec_earth3_sensitivity_growth)
...
# Run projections in parallel
ibm_monteratorero_ec_earth3_sensitivity_flowering = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_ec_earth3_sensitivity_flowering, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_ec_earth3_sensitivity_flowering)
...
# Run projections in parallel
ibm_monteratorero_ec_earth3_sensitivity_nbFlowers = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_ec_earth3_sensitivity_nbFlowers, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_ec_earth3_sensitivity_nbFlowers)
...
# Run projections in parallel
ibm_monteratorero_ec_earth3_sensitivity_seedlingSize = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_ec_earth3_sensitivity_seedlingSize, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_ec_earth3_sensitivity_seedlingSize)
...
# Run projections in parallel
ibm_monteratorero_fgoals_g3_sensitivity_survival = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_fgoals_g3_sensitivity_survival, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_fgoals_g3_sensitivity_survival)
...
# Run projections in parallel
ibm_monteratorero_fgoals_g3_sensitivity_growth = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_fgoals_g3_sensitivity_growth, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_fgoals_g3_sensitivity_growth)
...
# Run projections in parallel
ibm_monteratorero_fgoals_g3_sensitivity_flowering = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_fgoals_g3_sensitivity_flowering, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_fgoals_g3_sensitivity_flowering)
...
# Run projections in parallel
ibm_monteratorero_fgoals_g3_sensitivity_nbFlowers = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_fgoals_g3_sensitivity_nbFlowers, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_fgoals_g3_sensitivity_nbFlowers)
...
# Run projections in parallel
ibm_monteratorero_fgoals_g3_sensitivity_seedlingSize = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_fgoals_g3_sensitivity_seedlingSize, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_fgoals_g3_sensitivity_seedlingSize)
...
# Run projections in parallel
ibm_monteratorero_gfdl_esm4_sensitivity_survival = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_gfdl_esm4_sensitivity_survival, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_gfdl_esm4_sensitivity_survival)
...
# Run projections in parallel
ibm_monteratorero_gfdl_esm4_sensitivity_growth = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_gfdl_esm4_sensitivity_growth, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_gfdl_esm4_sensitivity_growth)
...
# Run projections in parallel
ibm_monteratorero_gfdl_esm4_sensitivity_flowering = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_gfdl_esm4_sensitivity_flowering, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_gfdl_esm4_sensitivity_flowering)
...
# Run projections in parallel
ibm_monteratorero_gfdl_esm4_sensitivity_nbFlowers = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_gfdl_esm4_sensitivity_nbFlowers, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_gfdl_esm4_sensitivity_nbFlowers)
...
# Run projections in parallel
ibm_monteratorero_gfdl_esm4_sensitivity_seedlingSize = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_gfdl_esm4_sensitivity_seedlingSize, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_gfdl_esm4_sensitivity_seedlingSize)
...
# Run projections in parallel
ibm_monteratorero_giss_e2_1_g_sensitivity_survival = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_giss_e2_1_g_sensitivity_survival, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_giss_e2_1_g_sensitivity_survival)
...
# Run projections in parallel
ibm_monteratorero_giss_e2_1_g_sensitivity_growth = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_giss_e2_1_g_sensitivity_growth, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_giss_e2_1_g_sensitivity_growth)
...
# Run projections in parallel
ibm_monteratorero_giss_e2_1_g_sensitivity_flowering = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_giss_e2_1_g_sensitivity_flowering, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_giss_e2_1_g_sensitivity_flowering)
...
# Run projections in parallel
ibm_monteratorero_giss_e2_1_g_sensitivity_nbFlowers = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_giss_e2_1_g_sensitivity_nbFlowers, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_giss_e2_1_g_sensitivity_nbFlowers)
...
# Run projections in parallel
ibm_monteratorero_giss_e2_1_g_sensitivity_seedlingSize = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_giss_e2_1_g_sensitivity_seedlingSize, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_giss_e2_1_g_sensitivity_seedlingSize)
...
# Run projections in parallel
ibm_monteratorero_inm_cm4_8_sensitivity_survival = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_inm_cm4_8_sensitivity_survival, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_inm_cm4_8_sensitivity_survival)
...
# Run projections in parallel
ibm_monteratorero_inm_cm4_8_sensitivity_growth = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_inm_cm4_8_sensitivity_growth, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_inm_cm4_8_sensitivity_growth)
...
# Run projections in parallel
ibm_monteratorero_inm_cm4_8_sensitivity_flowering = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_inm_cm4_8_sensitivity_flowering, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_inm_cm4_8_sensitivity_flowering)
...
# Run projections in parallel
ibm_monteratorero_inm_cm4_8_sensitivity_nbFlowers = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_inm_cm4_8_sensitivity_nbFlowers, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_inm_cm4_8_sensitivity_nbFlowers)
...
# Run projections in parallel
ibm_monteratorero_inm_cm4_8_sensitivity_seedlingSize = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_inm_cm4_8_sensitivity_seedlingSize, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_inm_cm4_8_sensitivity_seedlingSize)
...
# Run projections in parallel
ibm_monteratorero_ipsl_cm6a_lr_sensitivity_survival = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_ipsl_cm6a_lr_sensitivity_survival, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_ipsl_cm6a_lr_sensitivity_survival)
...
# Run projections in parallel
ibm_monteratorero_ipsl_cm6a_lr_sensitivity_growth = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_ipsl_cm6a_lr_sensitivity_growth, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_ipsl_cm6a_lr_sensitivity_growth)
...
# Run projections in parallel
ibm_monteratorero_ipsl_cm6a_lr_sensitivity_flowering = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_ipsl_cm6a_lr_sensitivity_flowering, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_ipsl_cm6a_lr_sensitivity_flowering)
...
# Run projections in parallel
ibm_monteratorero_ipsl_cm6a_lr_sensitivity_nbFlowers = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_ipsl_cm6a_lr_sensitivity_nbFlowers, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_ipsl_cm6a_lr_sensitivity_nbFlowers)
...
# Run projections in parallel
ibm_monteratorero_ipsl_cm6a_lr_sensitivity_seedlingSize = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_ipsl_cm6a_lr_sensitivity_seedlingSize, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_ipsl_cm6a_lr_sensitivity_seedlingSize)
...
# Run projections in parallel
ibm_monteratorero_miroc6_sensitivity_survival = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_miroc6_sensitivity_survival, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_miroc6_sensitivity_survival)
...
# Run projections in parallel
ibm_monteratorero_miroc6_sensitivity_growth = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_miroc6_sensitivity_growth, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_miroc6_sensitivity_growth)
...
# Run projections in parallel
ibm_monteratorero_miroc6_sensitivity_flowering = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_miroc6_sensitivity_flowering, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_miroc6_sensitivity_flowering)
...
# Run projections in parallel
ibm_monteratorero_miroc6_sensitivity_nbFlowers = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_miroc6_sensitivity_nbFlowers, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_miroc6_sensitivity_nbFlowers)
...
# Run projections in parallel
ibm_monteratorero_miroc6_sensitivity_seedlingSize = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_miroc6_sensitivity_seedlingSize, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_miroc6_sensitivity_seedlingSize)
...
# Run projections in parallel
ibm_monteratorero_mpi_esm1_2_lr_sensitivity_survival = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_mpi_esm1_2_lr_sensitivity_survival, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_mpi_esm1_2_lr_sensitivity_survival)
...
# Run projections in parallel
ibm_monteratorero_mpi_esm1_2_lr_sensitivity_growth = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_mpi_esm1_2_lr_sensitivity_growth, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_mpi_esm1_2_lr_sensitivity_growth)
...
# Run projections in parallel
ibm_monteratorero_mpi_esm1_2_lr_sensitivity_flowering = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_mpi_esm1_2_lr_sensitivity_flowering, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_mpi_esm1_2_lr_sensitivity_flowering)
...
# Run projections in parallel
ibm_monteratorero_mpi_esm1_2_lr_sensitivity_nbFlowers = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_mpi_esm1_2_lr_sensitivity_nbFlowers, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_mpi_esm1_2_lr_sensitivity_nbFlowers)
...
# Run projections in parallel
ibm_monteratorero_mpi_esm1_2_lr_sensitivity_seedlingSize = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_mpi_esm1_2_lr_sensitivity_seedlingSize, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_mpi_esm1_2_lr_sensitivity_seedlingSize)
...
# Run projections in parallel
ibm_monteratorero_mri_esm2_0_sensitivity_survival = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_mri_esm2_0_sensitivity_survival, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_mri_esm2_0_sensitivity_survival)
...
# Run projections in parallel
ibm_monteratorero_mri_esm2_0_sensitivity_growth = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_mri_esm2_0_sensitivity_growth, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_mri_esm2_0_sensitivity_growth)
...
# Run projections in parallel
ibm_monteratorero_mri_esm2_0_sensitivity_flowering = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_mri_esm2_0_sensitivity_flowering, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_mri_esm2_0_sensitivity_flowering)
...
# Run projections in parallel
ibm_monteratorero_mri_esm2_0_sensitivity_nbFlowers = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_mri_esm2_0_sensitivity_nbFlowers, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_mri_esm2_0_sensitivity_nbFlowers)
...
# Run projections in parallel
ibm_monteratorero_mri_esm2_0_sensitivity_seedlingSize = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_mri_esm2_0_sensitivity_seedlingSize, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_mri_esm2_0_sensitivity_seedlingSize)
...
# Run projections in parallel
ibm_monteratorero_noresm2_mm_sensitivity_survival = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_noresm2_mm_sensitivity_survival, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_noresm2_mm_sensitivity_survival)
...
# Run projections in parallel
ibm_monteratorero_noresm2_mm_sensitivity_growth = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_noresm2_mm_sensitivity_growth, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_noresm2_mm_sensitivity_growth)
...
# Run projections in parallel
ibm_monteratorero_noresm2_mm_sensitivity_flowering = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_noresm2_mm_sensitivity_flowering, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_noresm2_mm_sensitivity_flowering)
...
# Run projections in parallel
ibm_monteratorero_noresm2_mm_sensitivity_nbFlowers = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_noresm2_mm_sensitivity_nbFlowers, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_noresm2_mm_sensitivity_nbFlowers)
...
# Run projections in parallel
ibm_monteratorero_noresm2_mm_sensitivity_seedlingSize = sfClusterApplyLB(1:ncpus, startIBM)

# Save results and stop cluster
save(ibm_monteratorero_noresm2_mm_sensitivity_seedlingSize, 
     file = paste0("Output/Projections/IBM_Anthropogenic_SensitivityAnalysis_", 
                   population, "_", climate_model, "_", 
                   vital_rate, ".RData"))
sfStop()

rm(ibm_monteratorero_noresm2_mm_sensitivity_seedlingSize)
